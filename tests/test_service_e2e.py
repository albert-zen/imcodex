...
async def test_connection_ready_delivers_terminal_result_completed_during_disconnect() -> None:
    ...
    recovered = [
        ...
        if (
            message.message_type == "agentMessage"
...
@pytest.mark.asyncio
async def test_restart_does_not_repeat_an_acknowledged_answer_or_add_empty_fallback(
    tmp_path,
) -> None:
    state_path = tmp_path / "state.json"
    notification = {
    ...
    first_store = ConversationStore(clock=lambda: 1.0, state_path=state_path)
    first_store.bind_thread("qq", "conv-1", "thr_1")
    first_store.note_active_turn("thr_1", "turn_1", "inProgress")
    first_sink = CapturingSink()
    first_process = ScriptedProcess(
        {"initialize": [{"id": 1, "result": {"ok": True}}]}
    )
    first_client, first_service = _build_service(
        first_store,
        first_process,
        first_sink,
    )

    await first_service.handle_notification(notification)
    await first_store.flush_pending_writes()

    assert [message.text for message in first_sink.messages] == [
        "Already delivered."
    ]
    ...
    reloaded_store = ConversationStore(clock=lambda: 2.0, state_path=state_path)
    second_sink = CapturingSink()
    second_process = ScriptedProcess(
        {"initialize": [{"id": 1, "result": {"ok": True}}]}
    )
    second_client, second_service = _build_service(
        reloaded_store,
        second_process,
        second_sink,
    )

    replayed = await second_service.handle_notification(notification)
    completed = await second_service.handle_notification(
        {
            "method": "turn/completed",
            "params": {
                "threadId": "thr_1",
    ...
    assert replayed == []
    assert completed == []
    assert second_sink.messages == []
    assert reloaded_store.list_terminal_delivery_watches("thr_1") == []
    assert not reloaded_store.is_terminal_delivery_acknowledged(
        first_sink.messages[0].metadata["delivery_id"]
    )
...
async def test_recovered_terminal_result_remains_retryable_until_delivery_succeeds() -> None:
    ...
    assert sink.messages[0].message_type == "agentMessage"
    assert sink.messages[0].metadata["phase"] == "final_answer"
    assert sink.messages[0].metadata["native_item_type"] == "agentMessage"
    assert "native_item_id" not in sink.messages[0].metadata
...
@pytest.mark.asyncio
async def test_snapshot_recovery_delivers_each_final_answer_around_later_commentary() -> None:
    store = ConversationStore(clock=lambda: 1.0)
    store.bind_thread("qq", "conv-1", "thr_1")
    store.note_active_turn("thr_1", "turn_1", "inProgress")
    sink = CapturingSink()
    ...
    async def recover_failed_turn() -> dict:
        return {
                    ...
                    "turn": {
                        ...
                        "items": [
                            ...
                            },
                            {
                                ...
                                "type": "agentMessage",
                                "phase": "final_answer",
                                "text": "Second native final answer.",
                            },
                        ],
                    },
                }
    ...
    service.backend.rehydrate_bound_threads = recover_failed_turn  # type: ignore[method-assign]

    await service.handle_connection_ready(1)

    assert [message.text for message in sink.messages] == [
        "Native final answer.",
        "Second native final answer.",
    ]
    assert [message.message_type for message in sink.messages] == [
        "agentMessage",
        "agentMessage",
    ]
    assert [message.metadata["native_item_id"] for message in sink.messages] == [
        "answer_1",
        "answer_3",
    ]
    assert {
        message.metadata["native_item_type"] for message in sink.messages
    } == {"agentMessage"}
    assert {message.metadata["phase"] for message in sink.messages} == {
        "final_answer"
    }
    assert {message.metadata["status"] for message in sink.messages} == {"failed"}
    assert len(
        {message.metadata["delivery_id"] for message in sink.messages}
    ) == 2
    await service.close()
    await client.close()
...
@pytest.mark.asyncio
async def test_snapshot_recovery_delivers_artifacts_after_the_last_answer(
    tmp_path,
) -> None:
    ...
    workspace.mkdir()
    generated = workspace / "recovered.png"
    Image.new("RGB", (2, 2), (1, 2, 3)).save(generated)

    class ArtifactCheckingSink(CapturingSink):
        def __init__(self) -> None:
            super().__init__()
            self.artifacts_existed_at_send: list[bool] = []

        async def send_message(self, message: OutboundMessage) -> None:
            self.artifacts_existed_at_send.append(
                all(os.path.exists(artifact.local_path) for artifact in message.artifacts)
            )
            await super().send_message(message)

    store = ConversationStore(clock=lambda: 1.0)
    store.bind_thread_with_cwd(
        "qq",
        "conv-1",
        "thr_1",
        str(workspace),
                        ...
                        "items": [
                            {
                                ...
                                "type": "agentMessage",
                                "phase": "final_answer",
                                "text": "Answer before continued work.",
                            },
                            {
                                "id": "image_2",
    ...
    assert sink.messages[1].text == "Turn failed."
    assert [artifact.filename for artifact in sink.messages[1].artifacts] == [
        "recovered.png"
    ]
    assert sink.artifacts_existed_at_send == [True, True]
    assert len(
        {message.metadata["delivery_id"] for message in sink.messages}
    ) == 2
    await service.close()
...
@pytest.mark.asyncio
async def test_snapshot_recovery_fails_explicitly_for_colliding_no_id_answers() -> None:
    store = ConversationStore(clock=lambda: 1.0)
    store.bind_thread("qq", "conv-1", "thr_1")
    store.note_active_turn("thr_1", "turn_1", "inProgress")
    sink = CapturingSink()
    ...
    async def recover_ambiguous_turn() -> dict:
        return {
            "summary": {"total": 1, "succeeded": 1, "failed": 0, "unverified": 0},
            "recoveredTurns": [
                {
    ...
    service.backend.rehydrate_bound_threads = recover_ambiguous_turn  # type: ignore[method-assign]

    result = await service.handle_connection_ready(1)

    assert result is not None
    assert result["status"] == "degraded"
    assert result["rehydration"]["deliveryFailed"] == 1
    assert sink.messages == []
    assert ("thr_1", "turn_1") in service._pending_recovered_turns
    assert store.list_terminal_delivery_watches("thr_1")
    await service.close()
    await client.close()
...
async def test_recovery_keeps_turn_watch_until_recovered_message_is_durable() -> None:
    ...
    async def recover_completed_turn() -> dict:
        return {
            "summary": {"total": 1, "succeeded": 1, "failed": 0, "unverified": 0},
            "recoveredTurns": [
                {
    ...
    service.backend.rehydrate_bound_threads = recover_completed_turn  # type: ignore[method-assign]
    store.flush_pending_writes = fail_persistence  # type: ignore[method-assign]

    result = await service.handle_connection_ready(1)

    assert result is not None
    assert result["status"] == "degraded"
    assert result["rehydration"]["deliveryPending"] == 1
...
@pytest.mark.asyncio
async def test_live_completion_keeps_turn_watch_until_fallback_is_durable() -> None:
    store = ConversationStore(clock=lambda: 1.0)
    store.bind_thread("qq", "conv-1", "thr_1")
    store.note_active_turn("thr_1", "turn_1", "inProgress")
    process = ScriptedProcess({"initialize": [{"id": 1, "result": {"ok": True}}]})
...
@pytest.mark.asyncio
async def test_terminal_outbox_failure_blocks_only_the_same_im_destination() -> None:
    class FailFirstConversationSink:
        def __init__(self) -> None:
            self.messages: list[OutboundMessage] = []

        async def send_message(self, message: OutboundMessage) -> None:
            if message.conversation_id == "conv-1":
                raise OSError("conv-1 unavailable")
            self.messages.append(message)

    store = ConversationStore(clock=lambda: 1.0)
    for delivery_id, conversation_id in (
        ("delivery-1", "conv-1"),
        ("delivery-2", "conv-1"),
        ("delivery-3", "conv-2"),
    ):
    ...
    sink = FailFirstConversationSink()
    process = ScriptedProcess({"initialize": [{"id": 1, "result": {"ok": True}}]})
    client, service = _build_service(store, process, sink)  # type: ignore[arg-type]
    service._terminal_delivery_closed = True
    ...
    assert delivered is True
    assert [message.text for message in sink.messages] == ["delivery-3"]
    assert [
        pending.delivery_id for pending in store.list_pending_terminal_deliveries()
...
async def test_initial_terminal_failure_persists_partial_artifact_progress() -> None:
    ...
    _outbound, delivered, durable = await service._deliver_terminal_message(
        TerminalDeliveryIdentity("delivery-1", "thr_1", "turn_1"),
        message,
    )
...
async def test_permission_request_is_projected_and_approve_grants_requested_permissions() -> None:
    ...
    assert projected[0].message_type == "item/permissions/requestApproval"
...
async def test_native_work_after_final_resumes_live_output_for_the_same_turn() -> None:
    ...
    assert await service.handle_notification(
        {
            "method": "turn/completed",
            "params": {
                "threadId": "thr_1",
    ...
    assert [message.message_type for message in final_messages] == [
        "agentMessage",
        "agentMessage",
    ]
    assert {
        message.metadata.get("native_method") for message in final_messages
    } == {"item/completed"}
    assert {
        message.metadata.get("native_item_type") for message in final_messages
    } == {"agentMessage"}
    assert store.list_pending_terminal_deliveries() == []
    await service.close()
    await client.close()
...
@pytest.mark.asyncio
async def test_failed_answer_segment_does_not_suppress_later_output_in_same_turn() -> None:
    class FailFirstAnswerSink:
        def __init__(self) -> None:
            self.messages: list[OutboundMessage] = []
            self.fail_first = True

        async def send_message(self, message: OutboundMessage) -> None:
            if self.fail_first and message.text == "First answer.":
                raise OSError("first answer delivery failed")
            self.messages.append(message)
    ...
    await service.handle_notification(first_answer)
    await service.handle_notification(
        {
            "method": "item/started",
            "params": {
    ...
    # A native replay of the failed item must be ignored by item identity and
    # must not close the newly resumed projection segment.
    assert await service.handle_notification(first_answer) == []
    assert service.projector.message_pump._turns[
        ("thr_1", "turn_1")
    ].final_visible is False
    await service.handle_notification(
    ...
    pending = store.list_pending_terminal_deliveries()
    assert [(item.turn_id, item.message["text"]) for item in pending] == [
        ("turn_1", "First answer."),
        ("turn_1", "Second answer."),
    ]
    assert [watch.turn_id for watch in store.list_terminal_delivery_watches()] == [
        "turn_1"
    ]
    sink.fail_first = False
    assert await service._deliver_pending_terminal_once() is True
    assert [message.text for message in sink.messages] == [
        "Continuing after the queued steer.",
        "First answer.",
...
@pytest.mark.asyncio
async def test_active_writer_rehydration_retries_until_native_lock_is_released() -> None:
    conflict_thread_id = "019fe5f6-0000-7000-8000-000000000001"
            ...
            "thread/resume": [
                ...
                },
                {
                    "error": {
                        "code": -32600,
                        "message": (
                            f"thread {conflict_thread_id} already has an active writer"
                        ),
                    }
                },
                {
    ...
    store.bind_thread("qq", "conflict", conflict_thread_id)
    client, service = _build_service(store, process, CapturingSink())
    service.backend.prefers_native_recovery = lambda: True  # type: ignore[method-assign]
    retry_allowed = asyncio.Event()

    async def wait_for_native_writer_release(_delay_s: float) -> None:
        await retry_allowed.wait()

    service._native_writer_rehydration_retry._sleep = wait_for_native_writer_release

    await client.initialize()

    initial = client.connection_facts()
    assert initial["status"] == "degraded"
    assert initial["rehydration"] == {
        "total": 2,
        "succeeded": 1,
        "failed": 1,
        "unverified": 0,
    ...
    assert service._native_writer_rehydration_retry.running is True

    retry_allowed.set()
    await _wait_for_condition(
        lambda: not service._native_writer_rehydration_retry.running,
    )

    resumes = [
        payload for payload in process.inputs if payload.get("method") == "thread/resume"
    ]
    assert [payload["params"]["threadId"] for payload in resumes] == [
        "thr_healthy",
        conflict_thread_id,
        conflict_thread_id,
    ]
    recovered = client.connection_facts()
    assert recovered["status"] == "connected"
    assert recovered["rehydration"] == {
        "total": 2,
        "succeeded": 2,
        "failed": 0,
        "unverified": 0,
...
@pytest.mark.asyncio
async def test_ordinary_resume_invalidates_pending_active_writer_retry_before_turn_start() -> None:
    thread_id = "019fe5f6-0000-7000-8000-000000000004"
    process = SequentialScriptedProcess(
        {
            "initialize": [{"result": {"ok": True}}],
            "thread/resume": [
    ...
    store = ConversationStore(clock=lambda: 1.0)
    store.bind_thread_with_cwd("qq", "conflict", thread_id, "/work/current")
    client, service = _build_service(store, process, CapturingSink())
    service.backend.prefers_native_recovery = lambda: True  # type: ignore[method-assign]
    sleep_started = asyncio.Event()
    release_retry = asyncio.Event()

    async def blocked_retry_sleep(_delay_s: float) -> None:
        sleep_started.set()
        await release_retry.wait()

    service._native_writer_rehydration_retry._sleep = blocked_retry_sleep
    ...
    assert messages == []
    assert store.get_active_turn(thread_id) == ("turn_fresh", "inProgress")
    assert client.connection_facts()["rehydration"] == {
        "total": 1,
        "succeeded": 1,
        "failed": 0,
        "unverified": 0,
...
@pytest.mark.asyncio
async def test_connection_reset_cancels_pending_active_writer_retry() -> None:
    thread_id = "019fe5f6-0000-7000-8000-000000000002"
    process = SequentialScriptedProcess(
        {
            "initialize": [{"result": {"ok": True}}],
            "thread/resume": [
    ...
    store = ConversationStore(clock=lambda: 1.0)
    store.bind_thread("qq", "conflict", thread_id)
    client, service = _build_service(store, process, CapturingSink())
    service.backend.prefers_native_recovery = lambda: True  # type: ignore[method-assign]
    sleep_started = asyncio.Event()
    ...
    await client.initialize()
    await sleep_started.wait()
    await service.handle_connection_reset(client.connection_epoch)
    await asyncio.sleep(0)
    ...
    assert service._native_writer_rehydration_retry.running is False
    assert service._native_writer_rehydration_summary is None
    await service.close()
    await client.close()


@pytest.mark.asyncio
async def test_active_writer_retry_exhaustion_preserves_degraded_health() -> None:
    thread_id = "019fe5f6-0000-7000-8000-000000000003"
    conflict = {
        "error": {
            "code": -32600,
            "message": f"thread {thread_id} already has an active writer",
    ...
    process = SequentialScriptedProcess(
        {
            ...
            "turn/start": [
                {"result": {"turn": {"id": "turn_after_retry", "status": "inProgress"}}}
            ],
        }
    )
    store = ConversationStore(clock=lambda: 1.0)
    ...
    async def release_retries_after_initialize(_delay_s: float) -> None:
        await retry_allowed.wait()

    service._native_writer_rehydration_retry._delays_s = (0.0, 0.0)
    service._native_writer_rehydration_retry._sleep = release_retries_after_initialize
    ...
    messages = await service.handle_inbound(
        InboundMessage(
            ...
            conversation_id="conflict",
            user_id="u1",
            message_id="m-after-retry",
            text="continue after writer release",
        )
    )
    assert messages == []
    assert client.connection_facts()["status"] == "connected"
    assert client.connection_facts()["rehydration"] == {
        "total": 1,
        "succeeded": 1,
        "failed": 0,
    ...
    assert store.get_active_turn(thread_id) == ("turn_after_retry", "inProgress")
    await service.close()
    await client.close()
...
async def test_full_handoff_gate_preserves_native_server_request(
    ...
    assert sink.messages[1].message_type == "item/commandExecution/requestApproval"
...
async def test_handoff_retries_buffered_approval_before_rejecting_native_request(
    ...
    class FlakyApprovalSink(CapturingSink):
        ...
        async def send_message(self, message: OutboundMessage) -> None:
            if message.message_type == "item/commandExecution/requestApproval":
                self.attempted_delivery_ids.append(str(message.metadata.get("delivery_id") or ""))
                if len(self.attempted_delivery_ids) == 1:
...
async def test_handoff_preserves_wire_order_across_notification_and_request_lanes() -> None:
    ...
    assert sink.messages[2].message_type == "item/commandExecution/requestApproval"
    await client.close()
