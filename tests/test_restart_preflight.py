...
def test_preflight_validates_bind_host_and_every_enabled_channel(monkeypatch) -> None:
    ...
    def build_sdk_managed_channels(resolved_settings):
    ...
    monkeypatch.setattr(
        "imcodex.sdk_composition.build_sdk_managed_channels",
        build_sdk_managed_channels,
    )
    monkeypatch.setattr(
        "imcodex.composition.build_runtime",
        lambda *_args, **_kwargs: pytest.fail("preflight must not construct a runtime"),
    )
