...
- product slash-command names, command-owned subcommands/options, project display names, and Thread human-readable selectors MUST match case-insensitively while preserving original user values and native names for execution and display
- `/threads` MAY derive a bounded, presentation-only Git common-root label for local linked worktrees, but MUST preserve exact native `cwd`/`path` values for execution and MUST NOT persist a project registry or Desktop-owned pin state
- history display MUST use native turn APIs, preserve terminal and active turn statuses, preserve complete user and Codex message text without bridge-owned truncation, and paginate to the oldest available turn without becoming model context or a bridge-local transcript
...
when the App Server shares the bridge's filesystem namespace. TCP locality or
successful connection alone MUST NOT establish that capability. Native Windows
is the narrow TCP exception: a canonical `ws://127.0.0.1:<port>` target MAY be
accepted even when explicitly configured, but only after the project core
manifest, PID, exact listener owner, live Codex command, and health are verified.
Remote targets, TLS targets, hostname aliases, and loopback listeners without a
matching valid project core manifest remain image-ineligible. The capability
MUST be bound to the exact configured target. Each new TCP connection epoch MUST clear the
