...
- ordinary final-answer Markdown links to non-image local files MUST remain navigation references and MUST NOT be inferred as outbound attachments; local image references MAY retain automatic preview delivery, while generic files require structured native output or explicit standalone delivery
...
- current-thread standalone delivery MUST resolve the latest IM binding inside the running bridge; Agent-side launchers MUST NOT parse persisted bridge state, receive bot credentials, or persist a parallel thread-to-bot route
