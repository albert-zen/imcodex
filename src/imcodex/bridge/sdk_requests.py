...
from imagent.interaction.controllers import RequestPresentation
...
class ImcodexRequestPresenter:
    """Consumer wording and bounded handles over public SDK request types."""

    _MAX_PRESENTED_REQUESTS = 256

    def __init__(self) -> None:
        self._lock = RLock()
        self._requests: OrderedDict[tuple[ConversationRef, str], PresentedRequest] = OrderedDict()
        ...
        *,
        ...
        with self._lock:
            candidates = [
                ...
                and item.request.kind.value == kind
            ]
        if token:
            candidates = [
                request
    ...
    def approvals(self, conversation_ref: ConversationRef) -> tuple[ApprovalRequest, ...]:
        with self._lock:
            return tuple(
                ...
                for item in self._requests.values()
                if item.conversation_ref == conversation_ref
                and isinstance(item.request, ApprovalRequest)
            )
    ...
    def user_input_response(
        ...
        for question_id, values in answers.items():
            ...
            labels = {choice.label.casefold(): choice.choice_id for choice in question.choices}
            normalized[question_id] = tuple(
                value if value in choice_ids else labels.get(value.casefold(), value)
                for value in values
            )
        return UserInputResponse(normalized)

    def _remember(self, conversation_ref: ConversationRef, request: InteractiveRequest) -> None:
        ...
        with self._lock:
            ...
            while len(self._requests) > self._MAX_PRESENTED_REQUESTS:
    ...
    def _render(request: InteractiveRequest) -> tuple[str, bool]:
        ...
        if isinstance(request, ApprovalRequest):
            lines = [
                ...
                request.prompt,
                "Use /approve to allow, /deny to reject, or /cancel to cancel.",
