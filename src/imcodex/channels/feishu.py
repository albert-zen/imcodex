...
class FeishuChannelAdapter(BaseChannelAdapter):
    ...
    async def start(self) -> None:
        ...
        if not self.app_id or not self.app_secret:
            raise RuntimeError(
                "Feishu adapter requires IMCODEX_FEISHU_APP_ID and "
                "IMCODEX_FEISHU_APP_SECRET when enabled."
            )
        if not self.access_policy.has_allowed_users:
            logger.warning(
                "Feishu has no allowed user IDs; inbound messages will be denied. "
                "Set IMCODEX_FEISHU_ALLOWED_USER_IDS."
            )
        ...
        try:
            while not self._stop_event.is_set():
                ...
                try:
                    ...
                    logger.warning("Feishu connection failed; retrying in %.1fs: %s", delay, type(exc).__name__)
                    mark_channel_health(
                        "feishu",
                        connected=False,
                        status="reconnecting",
    ...
    def _create_sdk(self) -> object:
        ...
        try:
            ...
            raise RuntimeError(
                "Feishu support requires the optional dependency. "
                "Install imcodex with: pip install -e '.[feishu]'"
            ) from exc
    ...
    def _subscribe_sdk(self, sdk: object) -> None:
        ...
        try:
            ...
            subscriptions.append(
                sdk.on("reconnecting", lambda *_args: self._queue_health(False, "reconnecting"))
            )
            subscriptions.append(
                sdk.on("reconnected", lambda *_args: self._queue_health(True, "connected"))
            )
    ...
    def _queue_inbound(self, message: object) -> None:
        ...
        if loop is None or loop.is_closed() or self._stop_event.is_set():
            return
        loop.call_soon_threadsafe(self._spawn_inbound_task, message)

    def _spawn_inbound_task(self, message: object) -> None:
        if self._stop_event.is_set():
            return
        task = asyncio.create_task(self.handle_sdk_message(message))
        self._inbound_tasks.add(task)
        task.add_done_callback(self._finish_inbound_task)

    def _finish_inbound_task(self, task: asyncio.Task) -> None:
        ...
        try:
            ...
            logger.exception("Feishu inbound task failed")
